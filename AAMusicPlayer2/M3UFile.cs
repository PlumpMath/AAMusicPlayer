...
using System.IO;
using AAMusicPlayer2.Extensions;
        ...
        public void Load(string fileName, bool resolveRelativePaths = false)
        {
            ...
            using (var reader = new StreamReader(fileName))
            {
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    {
                        Uri path;
                        if (!Uri.TryCreate("file:///" + line, UriKind.RelativeOrAbsolute, out path))
                            throw new M3UException("Invalid entry path.");

                        if (!File.Exists(path.LocalPath))
                        {
                            if (path.LocalPath.ToString().Substring(0, 1) == "\\") path = new Uri(workingUri.LocalPath.Substring(0,2) + path.LocalPath.Substring(1));
                            else path = new Uri(workingUri + "\\" + path.LocalPath.ToString().Substring(1));
                        }
